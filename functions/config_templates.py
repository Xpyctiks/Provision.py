...
def create_php_config(filename: str) -> str:
  ...
  config = f"""[{filename}]
user = {current_app.config.get("WWW_USER")}
group = {current_app.config.get("WWW_GROUP")}
listen = /run/php/{filename}.sock
listen.owner = {current_app.config.get("WWW_USER")}
listen.group = {current_app.config.get("WWW_GROUP")}
listen.mode = 0660
...
chdir = {os.path.join(current_app.config.get("WEB_FOLDER"),filename)}
...
php_admin_value[open_basedir] = {os.path.join(current_app.config.get("WEB_FOLDER"),filename)}:/tmp
