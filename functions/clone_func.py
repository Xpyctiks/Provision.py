...
def start_clone(domain: str, source_site: str, selected_account: str, selected_server: str, realname: str,  web_folder: str, its_not_a_subdomain: bool = False) -> tuple[bool, str]:
  ...
  try:
    ...
    if cloudflare_certificate(domain,selected_account,selected_server,its_not_a_subdomain):
        ...
        if result != dstPath:
          ...
          return False, f"Error while copying {srcPath} to {dstPath}!"
        ...
        if not setupNginx(domain+".zip",has_subdomain):
          ...
          return False, "setupNginx() function returned an error!"
      ...
      logging.error("start_clone(): function cloudflare_certificate() returned an error!")
      return False, "function cloudflare_certificate() returned an error!"
  except Exception as msg:
    logging.error(f"start_clone() general error: {msg}")
    return False, f"general error: {msg}"
