...
def send_to_telegram(message: str, subject: str | None = None):
  if subject is None:
...
def send_job_report(message: str, subject: str | None = None):
  ...
  if not current_app.config.get("SEND_JOB_DONE_REPORTS", True):
    logging.info(f"send_job_report(): sendJobDoneReports is disabled - suppressing report: {subject or _default_subject()} {message}")
    return
