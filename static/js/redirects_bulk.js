...
(function () {
  ...
  const redirectFromInput = document.getElementById('redirectFromInput');
  ...
  accountSelect.addEventListener('change', function () {
    ...
    fetch('/redirects_bulk/account_data?account=' + encodeURIComponent(account))
  ...
  // ── Redirect path fields ─────────────────────────────────────────────────

  redirectFromInput.addEventListener('input', updateSubmitState);
  redirectToInput.addEventListener('input', updateSubmitState);

  // ── Helpers ────────────────────────────────────────────────────────────────

  function updateSubmitState() {
    const hasFrom    = redirectFromInput.value.trim() !== '';
    const hasTo      = redirectToInput.value.trim() !== '';
    const hasChecked = domainsContainer.querySelectorAll('.domain-check:checked').length > 0;
    submitBtn.disabled = !(hasFrom && hasTo && hasChecked);
  }
